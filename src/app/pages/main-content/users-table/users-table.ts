...
@Component({
  ...
  imports: [MatTableModule, MatPaginatorModule, MatSortModule, DatePipe, FormsModule],
...
export class UsersTable {
  ...
  displayedColumns: string[] = ['avatar', 'name', 'age', 'genderDob', 'mail', 'action'];
  ...
  ngOnInit() {
    ...
    if (this.mode() === 'card') {
      this.displayedColumns = ['avatar', 'name', 'genderDob', 'mail', 'action'];
    }
  }
  ngAfterViewInit() {
  ...
  openUserModal(user?: User) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (user) {
        this.dataSource.data = this.dataSource.data.map((u) => (u.id === result.id ? result : u));
      } else {
        this.dataSource.data = [...this.dataSource.data, { ...result, id: Date.now() }];
      }
